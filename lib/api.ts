...
export async function analyzeBill(file: File, customerType: "Business" | "Residential"): Promise<AnalyzeBillResponse> {
...
export async function updateLineItem(
  ...
  const res = await fetchAPI(`${API_BASE}/update-line/${quoteId}?index=${index}`, {
    method: "POST",
...
export async function removeLineItem(quoteId: string, index: number): Promise<AnalyzeBillResponse> {
  const formData = new FormData()
...
export async function updateQuoteStatus(quoteId: string, status: string): Promise<AnalyzeBillResponse> {
  const formData = new FormData()
...
export async function sendEmail(quoteId: string, toEmail: string): Promise<{ message: string }> {
  const formData = new FormData()
