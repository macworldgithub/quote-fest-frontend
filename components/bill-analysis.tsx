...
export default function BillAnalysis({
  ...
  const displayRecommendations = recommendations.slice(0, 3).map((rec, idx) => ({
    id: idx,
    name: rec.name || `Option ${idx + 1}`,
    description: rec.description || "Custom plan",
    savings: Math.max(0, Math.round(currentSpend - (rec.new_monthly_ex || 0))),
    period: "/month",
    highlighted: idx === 0,
  }))
  ...
  if (displayRecommendations.length === 0) {
    return (
      <Card className="p-6 text-center space-y-3">
        ...
        <p className="text-muted-foreground">No recommendations yet. Upload a bill to get started.</p>
      </Card>
    )
  }
        ...
        {displayRecommendations.map((rec) => (
          <Card
            ...
            className={`p-4 cursor-pointer transition-all ${
              rec.highlighted ? "border-accent border-2 bg-accent/5" : "hover:border-accent/50"
            }`}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  {rec.highlighted && <Badge className="bg-accent text-accent-foreground">Recommended</Badge>}
                </div>
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-accent">
                  ${rec.savings}
                  <span className="text-xs text-muted-foreground font-normal">{rec.period}</span>
                </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {currentServices.length > 0 && (
              <div className="col-span-2">
                <p className="text-muted-foreground text-xs mb-2">Detected Services</p>
